...
function getReferences(currentNode, principalPrefix, wsdlRootTagName, attributePlaceHolder) {
  let referencesInNode = [],
    imports = getWSDLImports(currentNode, principalPrefix, wsdlRootTagName);
  imports.forEach((importInformation) => {
    let schemaLocation = getAttributeByNamePlaceHolder(importInformation, 'schemaLocation', attributePlaceHolder);
