...
function getNodeKiller(nodesData, isGlobal = false) {
  const {
    ...
    getSchemaProperties = (attributePlaceHolder) => {
      let properties = Object.keys(directParent.node).filter((key) => {
        return !key.includes(attributePlaceHolder) &&
          key !== current.key;
      });
      return properties;
    },
    shouldRemoveSchema = (attributePlaceHolder) => {
      const importsHasChildren = directParent.node.hasOwnProperty(current.key) ?
          directParent.node[current.key].length > 0 :
          false,
        schemaIsEmpty = getSchemaProperties(attributePlaceHolder).length === 0;
    ...
    nodeKiller = function(attributePlaceHolder) {
      let newValue = getArrayFrom(directParent.node[current.key]).filter((element) => {
        return element !== current.node;
      });
      directParent.node[current.key] = newValue;
      if (shouldRemoveSchema(attributePlaceHolder) && !isGlobal) {
        removeSchema();
      }
    };
  return nodeKiller;
