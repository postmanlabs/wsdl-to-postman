...
function getAttributeByName(parentNode, attName) {
  return getXMLAttributeByName(parentNode, PARSER_ATRIBUTE_NAME_PLACE_HOLDER, attName);
}
...
function getBindings(parsedXml, wsdlRoot, wsdlObject) {
  ...
  try {
    ...
    if (!bindings) {
      wsdlObject.log.logError(DOC_HAS_NO_BINDIGS_MESSAGE);
      return [];
    }
    return bindings;
  }
...
function getSchemaNamespace(parsedXml, wsdlRoot) {
  ...
  if (!schemaNamespace) {
    const principalPrefix = getPrincipalPrefix(parsedXml, wsdlRoot),
      ...
      schemaTag = getQNamePrefix(Object.keys(types).find((key) => {
...
function getBindingOperation(binding, principalPrefix, wsdlObject) {
  if (!binding || typeof binding !== 'object') {
    throw new WsdlError('Can not get operations from undefined or null object');
  }
  try {
    const operations = getArrayFrom(getNodeByName(binding, principalPrefix, OPERATION_TAG));
    if (!operations) {
      wsdlObject.log.logError(DOC_HAS_NO_BINDIGS_OPERATIONS_MESSAGE);
      return [];
    }
    return operations;
  }
  catch (error) {
    throw new WsdlError('Can not get binding operations from object');
  }
}
...
function getSecurity(parsedXml, securityNS, wsdlRoot) {
  ...
  if (!parsedXml || typeof parsedXml !== 'object') {
    throw new WsdlError('Can not get security from undefined or null object');
  }
  try {
    const principalPrefix = getPrincipalPrefix(parsedXml, wsdlRoot),
      definitions = getNodeByName(parsedXml, principalPrefix, wsdlRoot);
    ...
    throw new WsdlError('Can not get security from object');
  }
}
 ...
 */
function getOperationxPathInfo(bindingName, operationName, wsdlNamespaceUrl, wsdlRoot) {
  return {
    xpath: `//${wsdlRoot}//${BINDING_TAG}[@name="${bindingName}"]//${OPERATION_TAG}[@name="${operationName}"]`,
