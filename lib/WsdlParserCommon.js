 ...
 * @param {object} node the node that contains the nodeToKill
 * @param {string} key The key of the nodeToKill
 * @param {object} nodeToKill The node we will remove from the container
 * @returns {function} A method to remove the node from a different context but same references
 */
function getNodeKiller(node, key, nodeToKill) {
  const newValue = node[key].filter((element) => {
      return element !== nodeToKill;
    }),
    nodeKiller = function() {
      node[key] = newValue;
    };
              ...
              if (imports && importProperties.length > 0) {
                if (kill) {
                  ...
                  imports.forEach((element) => {
                    element.killNode = getNodeKiller(schema, key, element);
                  });
                }
