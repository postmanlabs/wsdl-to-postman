...
class Wsdl11Parser {
  ...
  getStyleFromBindingOperation(operation, bindingTagInfo) {
    ...
    try {
      if (bindingTagInfo.protocol === SOAP_PROTOCOL || bindingTagInfo.protocol === SOAP12_PROTOCOL) {
        return operation[bindingTagInfo.protocolPrefix + OPERATION_TAG][PARSER_ATRIBUTE_NAME_PLACE_HOLDER + ATRIBUTE_STYLE];
      }
    }
    catch (error) {
