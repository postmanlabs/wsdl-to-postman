...
class Wsdl11Parser {
  ...
  assignOperations(wsdlObject, parsedXml) {
    ...
    bindings.forEach((binding) => {
      // const bindingTagInfo = this.getBindingInfoFromBindinTag(binding, newWsdlObject.SOAPNamespace,
      // newWsdlObject.SOAP12Namespace),
      const operations = binding[principalPrefix + OPERATION_TAG];

      operations.forEach((operation) => {
        let wsdlOperation = new Operation(),
          name = operation[PARSER_ATRIBUTE_NAME_PLACE_HOLDER + 'name'];

        wsdlOperation.name = name;
  ...
  getBindingInfoFromBindinTag(binding, soapNamespace, soap12Namespace) {
    ...
    if (protocolKey === soap12Namespace.key) {
      protocol = 'SOAP12';
      verb = POST_METHOD;
    }

    if (protocolKey === soapNamespace.key) {
