...
class Wsdl11Parser {
  ...
  assignOperations(wsdlObject, parsedXml) {
    ...
    bindings.forEach((binding) => {
      const bindingTagInfo = this.getBindingInfoFromBindinTag(binding,
        ...
        bindingOperations = this.getArrayFrom(getNodeByName(binding, principalPrefix, OPERATION_TAG)),
        bindingTypeFilter =
      ...
      bindingOperations.forEach((operation) => {
        ...
        servicePort = this.getServicePortByBindingName(getAttributeByName(binding, NAME_TAG),
        ...
        wsdlOperation.portName = getAttributeByName(servicePort, NAME_TAG);
        ...
        wsdlOperation.url = getAttributeByName(address, LOCATION_TAG);
  ...
  getMessageElementNameByMessageName(parsedXml, principalPrefix, messageName) {
    let definitions = getNodeByName(parsedXml, principalPrefix, WSDL_ROOT),
      ...
      messages = this.getArrayFrom(getNodeByName(definitions, principalPrefix, MESSAGE_TAG)),
    ...
    if (foundMessage) {
      ...
      elementName = getAttributeByName(part, ATRIBUTE_ELEMENT).replace(THIS_NS_PREFIX, '');
    }
    return elementName;
  }
  ...
  getServicePortByBindingName(bindingName, services, principalPrefix) {
    ...
    try {
      ...
      services.forEach((service) => {
        const ports = this.getArrayFrom(getNodeByName(service, principalPrefix, PORT_TAG));
  ...
  getServiceByBindingName(bindingName, services, principalPrefix) {
    ...
    try {
      ...
      services.forEach((service) => {
        const ports = this.getArrayFrom(getNodeByName(service, principalPrefix, PORT_TAG));
        port = ports.find((port) => {
          return getAttributeByName(port, BINDING_TAG) === 'tns:' + bindingName;
        });
        if (port) {
  ...
  getPortTypeOperationByPortTypeNameAndOperationName(portTypeName, operationName,
    ...
    try {
      const definitions = getNodeByName(parsedXml, principalPrefix, WSDL_ROOT),
        arrayPortTypes = this.getArrayFrom(getNodeByName(definitions, principalPrefix, PORT_TYPE_TAG));
      portTypeFound = arrayPortTypes.find((portType) => {
        return getAttributeByName(portType, NAME_TAG) === portTypeName;
      });
      operations = this.getArrayFrom(getNodeByName(portTypeFound, principalPrefix, OPERATION_TAG));
  ...
  getStyleFromBindingOperation(operation, bindingTagInfo) {
    ...
    try {
      if (bindingTagInfo.protocol === SOAP_PROTOCOL ||
        ...
        return getAttributeByName(getNodeByName(operation, bindingTagInfo.protocolPrefix, OPERATION_TAG), ATRIBUTE_STYLE);
      }
    }
    catch (error) {
