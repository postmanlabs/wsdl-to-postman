...
class TransactionValidator {
  ...
  validateItems(items, wsdlObject, result) {
    ...
    items.forEach((requestItem) => {
      ...
      if (operationFromWSDL) {
        ...
        let headermiss = this.validateHeaders(requestItem.request.header)
        missmatches =
  ...
  validateHeaders(requestHeaders) {
    let missmatches = [],
      contentTypeMissmatch = this.validateContentTypeHeader(requestHeaders);
  ...
  validateContentTypeHeader(requestHeaders) {

    let contentTypeHeader = requestHeaders.find((header) => { return header.key === 'Content-Type' });
