...
class TransactionValidator {
  ...
  validateItems(items, wsdlObject, result) {
    ...
    items.forEach((requestItem) => {
      ...
      if (operationFromWSDL) {
        ...
        let headermiss = this.validateHeaders(requestItem.request.header, requestItem.request.id, false);
        missmatches =
          missmatches.concat(headermiss);
        ...
        requestItem.response.forEach((responseInItem) => {
          ...
          let headermiss = this.validateHeaders(responseInItem.header, responseInItem.id, true);
          missmatches =
            missmatches.concat(headermiss);
  ...
  validateContentTypeHeader(headers, entityId, isResponse) {
    ...
    let index = headers.findIndex((header) => { return header.key === 'Content-Type' });
    let contentTypeHeader = headers[index];
    if (!contentTypeHeader) {
      ...
      if (!contentTypeHeader.value.includes('text/xml')) {
        return {
          property: 'HEADER',
          transactionJsonPath: isResponse ? `$.responses[${entityId}].header[${index}].value` : `$.request.header[${index}].value`,
          schemaJsonPath: 'schemaPathPrefix',
