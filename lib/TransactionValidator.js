...
class TransactionValidator {
  ...
  addSuggestedIfRequired(mismatch, suggestAvailableFixes) {
    let newMismatch = JSON.parse(JSON.stringify(mismatch));
    if(suggestAvailableFixes === true) {
    ...
    if (body) {
      ...
      if (errors.length > 0) {
        ...
        mismatches = filteredErrors.map((error) => {
          return this.addSuggestedIfRequired(
            this.getBodyMismatchObject(operationFromWSDL, error, isResponse, options),
            suggestAvailableFixes
          );
        });
      }
