...
class TransactionValidator {
  ...
  validateItems(items, wsdlObject, xmlParser, options) {
    ...
    items.forEach((requestItem) => {
      const requestBody = requestItem.request.body ? requestItem.request.body.raw : '';
      let endpoints = [],
   ...
   * @param {boolean} isResponse define if the operation is from a request or a response
        ...
        mismatches = filteredErrors.map((error) => {
          ...
          if (suggestAvailableFixes) {
            if (detailedBlobValidation) {
              ...
              mismatch = getMismatchWithSuggestedFix(mismatch, rawXmlMessage, bodyFromSchema, detailedBlobValidation, cleanSchema);
            }
            else {
              bodyFromSchema = this.getBodyFromSchema(wsdlObject, operationFromWSDL, isResponse, xmlParser, false);
              mismatch = getMismatchWithSuggestedFix(mismatch, body, bodyFromSchema, detailedBlobValidation, cleanSchema);
            }
          }
          return mismatch;
        });
