...
class TransactionValidator {
  ...
  validateItems(items, wsdlObject, result) {
    ...
    items.forEach((requestItem) => {
      ...
      let processedInfo = this.getItemRequestProcessedInfo(requestItem.request),
        operationFromWSDL = this.getOperationFromWSDLObjectByUrlNameProtocolMethod(processedInfo, wsdlObject);
    ...
    localResult.missingEndpoints = this.getMissingCollectionEndpoints(wsdlObject.operationsArray, processedInfoItems);
    return localResult;
  }
   ...
   * @returns {object} validation results
   */
  getOperationFromWSDLObjectByUrlNameProtocolMethod(itemRequestProcessedInfo, wsdlObject) {
  ...
  getMissingCollectionEndpoints(wsdlOperationsArray, processedInfoItems) {
    ...
    wsdlOperationsArray.forEach((operation) => {
      let foundItem = processedInfoItems.find((itemRequestProcessedInfo) => {
        return operation.url === itemRequestProcessedInfo.url &&
