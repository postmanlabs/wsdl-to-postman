...
class TransactionValidator {
  ...
  getMismatchWithSuggestedFix(mismatch, currentBody, bodyFromSchema, detailedBlobValidation) {
    ...
    if (!detailedBlobValidation) {
      ...
      const isMissingInRequest = reasonCode === MISSING_IN_REQUEST_REASON_CODE;
  ...
  getElementLineFromBody(body, elementKey) {
    const getLine = (body, elementKey) => {
        const splittedBody = body.split('\n'),
          elementLine = splittedBody.find((bodyLine) => {
            return bodyLine.includes(`<${elementKey}>`);
          });
        return elementLine;
      },
      elementInBody = body.includes(`<${elementKey}>`);
    return elementInBody ? getLine(body, elementKey) : '';
  }

  getBodyFromSchema(wsdlObject, operationFromWsdl) {
    const messageHelper = new SOAPMessageHelper(),
      message = messageHelper.convertInputToMessage(
        operationFromWsdl.input,
      ...
      if (errors.length > 0) {
        ...
        mismatches = filteredErrors.map((error) => {
          ...
          if (suggestAvailableFixes) {
            bodyFromSchema = this.getBodyFromSchema(wsdlObject, operationFromWSDL);
