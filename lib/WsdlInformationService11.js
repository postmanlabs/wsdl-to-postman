...
class WsdlInformationService11 {
  ...
  getBindingOperationName(bindingOperation) {
    return getAttributeByName(bindingOperation, ATRIBUTE_NAME);
  }

  getPortTypeInterfaceName(binding) {
    return getAttributeByName(binding, ATRIBUTE_TYPE).replace(THIS_NS_PREFIX, '');
  }
   ...
   */
  getServiceAndServicePortEndpointByBindingName(bindingName, services, principalPrefix, wsdlObject) {
    if (!bindingName) {
   ...
   */
  getPortTypeInterfaceOperationByPortTypeNameAndOperationName(portTypeName, operationName,
