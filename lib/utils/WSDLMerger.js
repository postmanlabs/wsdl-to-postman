 ...
 */
   ...
   * @param {object} rootPrincipalPrefix the principla prefix of root file
   * @returns {object} the root
   */
  resolveImportsFromRootFile(WSDLRootFile, parsedWSDL, parsedSchemas, wsdlRoot, attributePlaceHolder,
    ...
    parentFolder =
      getFolderNameFromPath(parsedXMLsAndFileNames
        .find((tuple) => { return tuple.parsed === processingRoot; }).fileName);
  ...
  assignImportedPropertiesSchema(root, importedTag, parsedWSDL, parsedSchemas, wsdlRoot, principalPrefix,
    ...
    if (!existingTypes || existingTypes.length === 0) {
      ...
      let schemasInType = getNodeByQNameLocalArray(existingTypes[0], SCHEMA_TAG).filter((schema) => {
        return schema !== importedTag;
      });
      if (schemasInType.length === 0) {
        existingTypes[0][schemaPrefix + SCHEMA_TAG] = schemaInType;
      }
      else {
        schemasInType.push(schemaProperty);
        // schemasInType.forEach((schemaInType) => {
        let typesInRoot = root[rootPrincipalPrefix + wsdlRoot][rootPrincipalPrefix + ATTRIBUTE_TYPES],
          targetType,
          schemasFromType;
