...
const path = (typeof window === 'object' && window.SDK_PLATFORM === 'browser') ?
    require('path-browserify') : require('path'),
  {
    getQNameLocal,
    XML_NAMESPACE_SEPARATOR
  } = require('./XMLParsedUtils'),
  fs = (typeof window === 'object' && window.SDK_PLATFORM === 'browser') ? require('browserify-fs') : require('fs'),
  CODIFICATION = 'utf8',
  {
    getNodeByName,
    getPrincipalPrefix,
