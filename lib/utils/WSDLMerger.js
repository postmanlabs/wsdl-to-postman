...
class WSDLMerger {
  ...
  getRoot(xmlParsedArray, wsdlRoot) {
    let root = xmlParsedArray.find((xmlParsed) => {
      let principalPrefix = getPrincipalPrefix(xmlParsed, wsdlRoot),
        hasImports = this.wsdlHasImports(xmlParsed, principalPrefix, wsdlRoot),
  ...
  wsdlHasImports(xmlParsed, principalPrefix, wsdlRoot) {
    return this.getWSDLImports(xmlParsed, principalPrefix, wsdlRoot).length > 0;
  }

  getWSDLImports(xmlParsed, principalPrefix, wsdlRoot) {
    let types,
      globalImports = [];

    if (xmlParsed[principalPrefix + wsdlRoot][IMPORT_TAG]) {
      globalImports.push(xmlParsed[principalPrefix + wsdlRoot][IMPORT_TAG]);
    }
    xmlParsed[principalPrefix + wsdlRoot][principalPrefix + 'types'];
    types = getArrayFrom(getNodeByName(xmlParsed[principalPrefix + wsdlRoot], principalPrefix, 'types'));
  ...
  resolveImportsFromRootFile(root, parsedWSDL, parsedSchemas, wsdlRoot) {
    let principalPrefix = getPrincipalPrefix(root, wsdlRoot),
      imports = this.getWSDLImports(root, principalPrefix, wsdlRoot);
  ...
  assignImportedPropertiesSchema(
    ...
    let schemaProperty = importedTag[SCHEMA_TAG],
      existingTypes = getArrayFrom(
        getNodeByName(root[principalPrefix + wsdlRoot], principalPrefix, 'types')
      );

    if (!existingTypes || existingTypes.length === 0) {
      root[principalPrefix + wsdlRoot][principalPrefix + 'types'] = [];
      root[principalPrefix + wsdlRoot][principalPrefix + 'types'].push(importedTag);
    }

    else {
      let schemasInType = getArrayFrom(getNodeByName(existingTypes[0], 'xsd:', SCHEMA_TAG));
      ...
      schemasInType.forEach((schemaInType) => {
        ...
        if (Array.isArray(root[principalPrefix + wsdlRoot][principalPrefix + 'types'])) {
          root[principalPrefix + wsdlRoot][principalPrefix + 'types'][0][schemaPrefix + SCHEMA_TAG] =
            schemaInType;
        }
        else {
          root[principalPrefix + wsdlRoot][principalPrefix + 'types'][schemaPrefix + SCHEMA_TAG] =
            schemaInType;
        }
      });
