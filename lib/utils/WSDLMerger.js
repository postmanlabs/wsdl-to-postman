 ...
 */
  ...
  setReferenceMapSchemas(WSDLRootFile, rootPrincipalPrefix, wsdlRoot, resolvedSchemas) {
    ...
    resolvedSchemas.forEach((resolvedSchema) => {
      schemasFromType = targetType[resolvedSchema.schemaPrefix + SCHEMA_TAG];
      if (Array.isArray(schemasFromType)) {
        ...
        if (found) {
          this.setReferenceMap(this.getxPathSchema(rootPrincipalPrefix + wsdlRoot,
            ...
            resolvedSchema.schemaPrefix + SCHEMA_TAG, indexFound), resolvedSchema.fileName);
        }
      }
      else if (schemasFromType) {
        let same = schemasFromType === resolvedSchema.found[resolvedSchema.schemaPrefix + SCHEMA_TAG];
        if (same) {
          this.setReferenceMap(this.getxPathSchema(rootPrincipalPrefix + wsdlRoot,
            rootPrincipalPrefix + ATTRIBUTE_TYPES,
            resolvedSchema.schemaPrefix + SCHEMA_TAG, 0), resolvedSchema.fileName);
        }
      }
    });
    ...
    includes.forEach((importInformation) => {
      ...
      if (importedFound.found) {
        this.assignImportedProperties(processingRoot, importedFound, parsedWSDL, parsedSchemas, wsdlRoot,
          principalPrefix, attributePlaceHolder, wsdlParser, parsedXMLsAndFileNames, importedFound.type,
          rootPrincipalPrefix);
        resolvedIncludes.push(importInformation);
      }
      else {
        delete importInformation.killNode;
      }
    });
    resolvedIncludes.forEach((resolved) => {
      if (resolved.hasOwnProperty('killNode')) {
        resolved.killNode();
      }
    });
    this.setReferenceMapSchemas(WSDLRootFile, rootPrincipalPrefix, wsdlRoot, this.resolvedSchemas);
    return WSDLRootFile;
  }
   ...
   * @param {object} nameAttribute the value of the attribute name of the imported tag
   * @returns {string} xpath
   */
  getxPath(wsdlRootTagname, importedProperty, nameAttribute) {
    return `//${wsdlRootTagname}//${importedProperty}[@${'name'}="${nameAttribute}"]`;
  }
  ...
  mergeProperty(root, principalPrefix, wsdlRoot, rootProperty,
    ...
    this.setReferenceMap(this.getxPath(principalPrefix + wsdlRoot, principalPrefix + importedProperty,
    ...
    if (!existingTypes || existingTypes.length === 0) {
      ...
      if (!hasSchemas) {
        this.assignImportedPropertiesSchemaTypesExistEmpty(existingTypes, schemaPrefix, schemaProperty,
          rootPrincipalPrefix, wsdlRoot, importedTagInformation);
      }
      else {
 ...
 */
  ...
  assignImportedPropertiesSchemaNotTypesTag(root, rootPrincipalPrefix, wsdlRoot, importedTagInformation) {
    ...
    // this.setReferenceMap(this.getxPathSchema(rootPrincipalPrefix + wsdlRoot, rootPrincipalPrefix + ATTRIBUTE_TYPES,
    //   schemaPrefix + SCHEMA_TAG, 0), importedTagInformation.fileName);
   ...
   * @returns {object} undefined
   */
  assignImportedPropertiesSchemaTypesExistEmpty(existingTypes, schemaPrefix, schemaProperty,
    rootPrincipalPrefix, wsdlRoot, importedTagInformation) {
   ...
   * @returns {object} undefined
   */
  assignImportedPropertiesSchemaTypesHasSchema(schemaPrefix, schemaProperty, existingTypes,
    rootPrincipalPrefix, wsdlRoot, importedTagInformation) {
    ...
    if (Array.isArray(schemasFromType)) {
      ...
      schemasToPush.forEach((schema) => {
        let index = schemasFromType.length;
        this.setReferenceMap(this.getxPathSchema(rootPrincipalPrefix + wsdlRoot, rootPrincipalPrefix + ATTRIBUTE_TYPES,
          schemaPrefix + SCHEMA_TAG, index), importedTagInformation.fileName);
      ...
      if (!wasMerged) {
        ...
        this.setReferenceMap(this.getxPathSchema(rootPrincipalPrefix + wsdlRoot, rootPrincipalPrefix + ATTRIBUTE_TYPES,
          schemaPrefix + SCHEMA_TAG, 1), importedTagInformation.fileName);
      }
    }
    else {
      existingTypes[0][schemaPrefix + SCHEMA_TAG] = schemaProperty;
