 ...
 */
    ...
    includes.forEach((importInformation) => {
      ...
      if (importedFound.found) {
        this.assignImportedProperties(processingRoot, importedFound, parsedWSDL, parsedSchemas, wsdlRoot,
          attributePlaceHolder, wsdlParser, parsedXMLsAndFileNames, importedFound.type,
          rootPrincipalPrefix);
      }
    });
   ...
   * @param {object} importedProperty the name of the property to merge
   * @param {object} nameAttribute the value of the attribute name of the imported tag
   * @param {object} fileName the name of the file where the imported tag was taken
   * @returns {undefined} nothing
   */
  setReferenceMap(wsdlRootTagname, importedProperty, nameAttribute, fileName) {
    let xPath = this.getxPath(wsdlRootTagname, importedProperty, nameAttribute);
  ...
  mergeProperty(root, principalPrefix, wsdlRoot, rootProperty,
    ...
    this.setReferenceMap(principalPrefix + wsdlRoot, principalPrefix + importedProperty,
