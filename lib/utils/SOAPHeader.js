const PASSWORD_TEXT = 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText',
  ...
  processUsernameToken = (usernameTokenInput, jObj) => {
    ...
    if (usernameTokenInput.passwordType === NORMAL_PASSWORD_TAG) {
      ...
      password[`${this.parserAttributePlaceHolder + ATTRIBUTE_TYPE}`] = PASSWORD_TEXT;
      password[ATTRIBUTE_TEXT_VALUE] = 'place password here';
      nonce[`${this.parserAttributePlaceHolder + ATTRIBUTE_ENCODING_TYPE}`] = '...#Base64Binary';
      ...
      usernameTokenOutput[`${WSSE_FILTER + USERNAME_TAG}`] = 'place username here';
      password[`${this.parserAttributePlaceHolder + ATTRIBUTE_TYPE}`] = PASSWORD_DIGEST;
  ...
  processSAMLToken = (samlTokenInput, jObj) => {
    ...
    samlTokenOutput[`${this.parserAttributePlaceHolder + ATTRIBUTE_ID}`] = 'place id here';
    samlTokenOutput[this.parserAttributePlaceHolder + ATTRIBUTE_ISSUE_INSTANT] = 'place issue instant';
    samlTokenOutput[this.parserAttributePlaceHolder + ATTRIBUTE_VERSION] = '2.0';
    samlTokenOutput[SAML2_FILTER + ISSUER_TAG] = SAML2_ISSUER;

    conditions[this.parserAttributePlaceHolder + ATTRIBUTE_NOT_BEFORE] = 'place not before';
    conditions[this.parserAttributePlaceHolder + ATTRIBUTE_NOT_ON_OR_AFTER] = 'place not on or after';

    nameID[this.parserAttributePlaceHolder + ATTRIBUTE_FORMAT] = SAML2_FORMAT;
    nameID[ATTRIBUTE_TEXT_VALUE] = 'joe,ou=people,ou=saml demo,o=example.com';

    subjectConfirmation[this.parserAttributePlaceHolder + ATTRIBUTE_METHOD] = mode;
  ...
  processSSLTransport = (transportBindingAssertion, jObj) => {
    if (transportBindingAssertion.includeTimestamp) {
      let timestamp = {};
      timestamp[this.parserAttributePlaceHolder + ATTRIBUTE_XMLNS + WSU] = WSU_URL;
