const PASSWORD_TEXT = 'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText',
  ...
  processUsernameToken = (usernameTokenInput, jObj) => {
    ...
    if (usernameTokenInput.passwordType === NORMAL_PASSWORD_TAG) {
      ...
      password[`${PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_TYPE}`] = PASSWORD_TEXT;
      password[ATTRIBUTE_TEXT_VALUE] = 'place password here';
      nonce[`${PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_ENCODING_TYPE}`] = '...#Base64Binary';
      ...
      usernameTokenOutput[`${WSSE_FILTER + USERNAME_TAG}`] = 'place username here';
      password[`${PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_TYPE}`] = PASSWORD_DIGEST;
  ...
  processSAMLToken = (samlTokenInput, jObj) => {
    ...
    samlTokenOutput[`${PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_ID}`] = 'place id here';
    samlTokenOutput[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_ISSUE_INSTANT] = 'place issue instant';
    samlTokenOutput[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_VERSION] = '2.0';
    samlTokenOutput[SAML2_FILTER + ISSUER_TAG] = SAML2_ISSUER;

    conditions[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_NOT_BEFORE] = 'place not before';
    conditions[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_NOT_ON_OR_AFTER] = 'place not on or after';

    nameID[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_FORMAT] = SAML2_FORMAT;
    nameID[ATTRIBUTE_TEXT_VALUE] = 'joe,ou=people,ou=saml demo,o=example.com';

    subjectConfirmation[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_METHOD] = mode;
    subject[SAML2_FILTER + NAMED_ID_TAG] = nameID;
  ...
  processSSLTransport = (transportBindingAssertion, jObj) => {
    if (transportBindingAssertion.includeTimestamp) {
      let timestamp = {};
      timestamp[PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_XMLNS + WSU] = WSU_URL;
...
class SOAPHeader {
  ...
  create(headerInfo, protocol) {
    ...
    jObj[WSSE_FILTER + SECURITY_TAG][`${PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + protocol + ':mustUnderstand'}`] = '1';
    jObj[WSSE_FILTER + SECURITY_TAG][`${PARSER_ATTRIBUTE_NAME_PLACE_HOLDER + ATTRIBUTE_XMLNS + WSSE}`] = OASIS_WS_SE;
