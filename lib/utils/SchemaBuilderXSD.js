...
class SchemaBuilderXSD {
  ...
  getWSDLElementsFromJsonSchema(jsonSchema, schemaTagInformation, parserPlaceholder, elements,
    ...
    if (jsonSchemaSimpleTypesFromCurrentTag && jsonSchemaSimpleTypesFromCurrentTag.length > 0) {
      this.processComplexOrSimpleTypesToElement(jsonSchemaSimpleTypesFromCurrentTag,
        jsonSchemaComplexTypesFromCurrentTag,
        jsonSchemaSimpleTypesFromCurrentTag, targetNamespace, simpleTypeElements);
    }
    ...
    if (jsonSchemaComplexTypesFromCurrentTag && jsonSchemaComplexTypesFromCurrentTag.length > 0) {
      this.processComplexOrSimpleTypesToElement(jsonSchemaComplexTypesFromCurrentTag,
        jsonSchemaComplexTypesFromCurrentTag,
        jsonSchemaSimpleTypesFromCurrentTag, targetNamespace, complexTypeElements);
    }
    // if (jsonSchemaSimpleTypesFromCurrentTag && jsonSchemaSimpleTypesFromCurrentTag.length > 0) {
    //   this.processComplexOrSimpleTypesToElement(jsonSchemaSimpleTypesFromCurrentTag,
    //     jsonSchemaComplexTypesFromCurrentTag,
    ...
    if (jsonSchemaElementsFromCurrentTag && jsonSchemaElementsFromCurrentTag.length > 0) {
      this.processElementsToWSDElements(jsonSchemaElementsFromCurrentTag, jsonSchemaComplexTypesFromCurrentTag,
        jsonSchemaSimpleTypesFromCurrentTag, targetNamespace, elements);
    }
    return {
      elements,
      simpleTypeElements,
   ...
   * @param {array} previousProcessedItems the list of previous processed simple complex or elements
   ...
   */
  getTypesFromDefinitions(parsedSchema, typesFromWSDL, previousProcessedItems, definitionsNames, parserPlaceholder) {
    let namesFromWSDL,
      definitionsElements,
      allNames = [],
    ...
    allNames = namesFromWSDL.concat(namesPreviousProcessed);
  ...
  getComplexTypeByName(complexTypeName, complexTypes) {
    ...
    if (complexTypeName.startsWith('FORWARD_REFERENCE#')) {
    ...
    else if (complexTypeName.startsWith('#/definitions/')) {
      ...
      complexType = complexTypes.find((complexType) => {
        return complexType.fullName === complexTypeName;
      });
    }
    return complexType;
  }
  ...
  getSimpleTypeByName(simpleTypeName, simpleTypes) {
    ...
    if (simpleTypeName.startsWith('FORWARD_REFERENCE#')) {
    ...
    else if (simpleTypeName.startsWith('#/definitions/')) {
      ...
      simpleType = simpleTypes.find((simpleType) => {
        return simpleType.fullName === simpleTypeName;
      });
    }
    return simpleType;
  }
