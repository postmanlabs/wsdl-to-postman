...
class SchemaBuilderXSD {
  ...
  getElementsFromType(type, parsedSchema, schemaPrefixFilter,
    ...
    elementTypesFromWSDL = this.getArrayFrom(this.getElementsFromSchema(scTag,
      schemaPrefixFilter));
    ...
    elementTypesNamesFromWSDL = elementTypesFromWSDL.map((simpleType) => {
      return getXMLAttributeByName(simpleType, parserPlaceholder, ATRIBUTE_NAME);
    });

    elementsFromWSDL = definitionsNames.filter((definitionName) => {
      return elementTypesNamesFromWSDL.includes(definitionName);
    }).map((definitionName) => {
      return {
  ...
  getElementsFromSchema(schemaTag, schemaPrefix) {
    return this.getArrayFrom(schemaTag[schemaPrefix + 'element']);
  }
