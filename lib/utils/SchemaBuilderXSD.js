...
class SchemaBuilderXSD {
  ...
  getXMLSchemaProcessed(schemaTag, schemaNamespace, tnsNamespaceURL, tnsNamespace, parserPlaceholder,
    ...
    if (schemaNamespace.key === '') {
      ...
      localSchemaTag[parserPlaceholder + 'xmlns' + XML_NAMESPACE_SEPARATOR + schemaNamespace.key] = schemaNamespace.url;
    }
    ...
    additionalNS.forEach((namespace) => {
      if (!localSchemaTag[parserPlaceholder + 'xmlns' + XML_NAMESPACE_SEPARATOR + namespace.key]) {
        localSchemaTag[parserPlaceholder + 'xmlns' + XML_NAMESPACE_SEPARATOR + namespace.key] = namespace.url;
      }
    });
    objectSchema = {};
  ...
  processElementsToWSDElements(elementsFromWSDL, complexTypes, simpleTypes, targetNamespace, elements) {
    ...
    sorted.forEach((element) => {
      ...
      this.getChildren(getDeepCopyOfObject(element), wsdlElement, elements, targetNamespace);
   ...
   * @param {boolean} cleanRereferences wheter to clean the already visited references array when the root has
   * been traversed
   * @returns {[Element]} the information of the types
   */
  searchAndAssignComplex(rootElement, complexTypes, simpleTypes, elements,
    lookedReferences, cleanRereferences) {
    let that = this;
  ...
  processReferenceTypeOfSchema(property, hasReferenceTypeKey, complexTypes, simpleTypes, elements,
    ...
    if (complexType) {
      ...
      if (elementType) {
        ...
        if (property.complexName !== property.properties.name &&
          !this.isLoopReferenceElement(property.properties, type, lookedReferences,
            this.isSelfReferenceElementExactNameComparer, property.complexName, path)) {
          lookedReferences.push({ element: property.complexName, path: path });
          this.searchAndAssignComplex(property.properties, complexTypes, simpleTypes,
