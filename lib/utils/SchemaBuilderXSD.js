...
class SchemaBuilderXSD {
  ...
  getElementsFromMesages(parsedXML, principalPrefix, elementsInTypes, simpleTypes, complexTypes, parserPlaceholder,
    ...
    filtered.forEach((message) => {
      ...
      parts = getArrayFrom(getXMLNodeByName(message, principalPrefix, ATRIBUTE_PART));
            ...
            if (properties[key]) {
              ...
              if (properties[key].type !== 'object' && !hasRef && isKnownType(properties[key].type)) {
                ...
                if (properties[key].type === ERROR_ELEMENT_IDENTIFIER) {
                  ...
                  let preprocessed = this.findIfPreviousProcessedElement(processedElements,
                    properties, key);
                  if (!preprocessed) {
                    element.name = key;
                    ...
                    element.maxOccurs = this.getMaxOccursOfElement(x, key);
                    wsdlElement.children.push(element);
                    //let virtualElement = this.assignVirtualProperties(x.properties[key], key);
                    this.getChildren(x.properties[key], element, processedElements);
                    x.properties[key] = {};
                  }
                  else {
  ...
  assignVirtualProperties(element, key) {
    if (!element.properties) {
      let result = {};
      result.properties = {};
      result.properties[key] = element
      return result;
    }
    return element;
  }
  ...
  findIfPreviousProcessedElement(preProcessedElements, properties, key) {
    let name = properties[key].complexName ? properties[key].complexName : properties[key].$ref.replace(DEFINITIONS_FILTER, '');
