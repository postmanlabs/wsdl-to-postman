...
class SchemaBuilderXSD {
  ...
  parseSchema(schema) {
    ...
    //console.log(jsonChema)
    console.log(schema)
    return jsonChema;
  }

  getXMLSchemaProcessed(schemaTag, schemaPrefix, schemaURL) {
    let localSchemaTag = {
      ...schemaTag
    };
    localSchemaTag[PARSER_ATRIBUTE_NAME_PLACE_HOLDER + 'xmlns:' + schemaPrefix.replace(':', '')] = schemaURL;
    let objectSchema = {};
    objectSchema[schemaPrefix + 'schema'] = localSchemaTag;
   ...
   */
  getElements(parsedXml, schema, principalPrefix, wsdlRoot, schemaPrefix, schemaURL) {
    ...
    types.forEach((type) => {
      ...
      schema = this.getXMLSchemaProcessed(schemaTag, schemaPrefix, schemaURL);
