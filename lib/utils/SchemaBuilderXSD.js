...
class SchemaBuilderXSD {
  ...
  processElementsToWSDElements(elementsFromWSDL, parsedSchema, complexTypes, simpleTypes, targetNamespace, elements) {
    let ordered = searchAndAssignComplex2(elementsFromWSDL);
            ...
            if (properties[key]) {
              ...
              if (properties[key].type !== 'object' && !hasRef && isKnownType(properties[key].type)) {
                ...
                if (properties[key].type === ERROR_ELEMENT_IDENTIFIER) {
                  ...
                  let preprocessed = this.getProcessedElement(processedElements, properties[key].complexName);
                  if (!preprocessed) {
                    element.name = key;
  ...
  getProcessedElement(processedElements, name) {
    if (!processedElements) {
      return;
    }
    const element = processedElements.find((current) => {
  ...
  getElementTypeByName(name, elements) {
    ...
    let elementType,
      typeName = name.replace('#/definitions/', '');
