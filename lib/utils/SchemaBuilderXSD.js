...
class SchemaBuilderXSD {
  ...
  getElements(parsedXml, principalPrefix, wsdlRoot, schemaNamespace, tnsNamespace, parserPlaceholder) {
    ...
    elementsFromMessages = this.getElementsFromMesages(parsedXml, principalPrefix,
      allElements.elements, allElements.simpleTypeElements, allElements.complexTypeElements, parserPlaceholder, wsdlRoot);
    ...
    filtered.forEach((message) => {
      ...
      parts.forEach((part) => {
        ...
        if (realPartTypeorelement) {
          let foundType = this.findType(realPartTypeorelement, elementsInTypes, simpleTypes, complexTypes);
          if (foundType.typeOfType === 'knownType') {
          ...
          if (foundType.typeOfType === 'isComplexType') {
          ...
          if (foundType.typeOfType === 'isSimpleType') {
            let type = foundType.type;
            if (type) {
              let element = this.getCopyOfElement(type);
              wsdlElement.children.push(element);
          ...
          if (foundType.typeOfType === 'isElement') {
            let type = foundType.type;
            if (type) {
              let element = this.getCopyOfElement(type);
              wsdlElement.children.push(element);
  ...
  findType(typeOrElementName, elementsInTypes, simpleTypes, complexTypes) {
    let found;
    if (isKnownType(typeOrElementName)) {
      return {
        typeOfType: 'knownType'
      };
    }
    if (elementsInTypes) {
  ...
  getElementsFromType(type, parsedSchema, schemaPrefixFilter,
    ...
    if (complexTypes) {
      complexTypes.forEach((element) => {
        ...
        this.searchAndAssignComplex(JSON.parse(JSON.stringify(element)), JSON.parse(JSON.stringify(parsedSchema)), complexTypes,
          simpleTypes);
        wsdlElement.namespace = targetNamespace;
    ...
    if (simpleTypes) {
      simpleTypes.forEach((element) => {
        let wsdlElement = new Element();
        this.searchAndAssignComplex(JSON.parse(JSON.stringify(element)), JSON.parse(JSON.stringify(parsedSchema)),
    ...
    if (elementsFromWSDL) {
      elementsFromWSDL.forEach((element) => {
        let wsdlElement = new Element();
        this.searchAndAssignComplex(element, JSON.parse(JSON.stringify(parsedSchema)),
          complexTypes,
          simpleTypes);
        wsdlElement.namespace = targetNamespace;
        wsdlElement.name = element.name;
