...
class SchemaBuilderXSD {
  ...
  getElements(parsedXml, principalPrefix, wsdlRoot, wsdlObject, parserPlaceholder) {
    ...
    types.forEach((type) => {
      ...
      const orderedSchemas = orderSchemasAccordingToDependencies(schemaTags);
      ...
      schemaTags.forEach((schema) => {
        let {
          elements,
          simpleTypeElements,
          complexTypeElements
        } = this.getWSDLElementsFromJsonSchema(mergedDefinitionsSchema, schema, parserPlaceholder);
        allElements.elements = allElements.elements.concat(elements);
        allElements.simpleTypeElements = allElements.simpleTypeElements.concat(simpleTypeElements);
        allElements.complexTypeElements = allElements.complexTypeElements.concat(complexTypeElements);
      });
    });
  ...
  mergeAllSchemaDefinitionsInOne(jsonSchemas) {
    ...
    if (jsonSchemas && jsonSchemas.length > 0) {
      mergedDefinitionsSchema = this.getDeepCopyOfObject(jsonSchemas[0]);
      var definitions = {};
  ...
  getSchemasInformation(typeElement, wsdlObject, parserPlaceholder) {
    ...
    if (schemasWithLocalDefinition) {
      schemaTags = schemaTags.concat(schemasWithLocalDefinition);
    }
    return schemaTags;
  }
      ...
      for (let index = 0; index < wsdlObject.localSchemaNamespaces.length; index++) {
        ...
        schemasInlocal.forEach((foundSchema) => {
          result.push({
            ...
            targetNamespace:
              getXMLAttributeByName(foundSchema,
      ...
      parts.forEach((part) => {
        ...
        if (realPartTypeorelement) {
          ...
          if (foundType.typeOfType === IS_COMPLEX_TYPE ||
            ...
            if (type) {
              let element = this.getShallowCopyOfObject(type);
              wsdlElement.children.push(element);
            }
          }
        }
   ...
   */
  getShallowCopyOfObject(element) {
    let newElement = {
      ...element
    };
    return newElement;
   ...
   * @returns {[Element]} the information of the types
   */
  getWSDLElementsFromJsonSchema(jsonSchema, schemaTagInformation, parserPlaceholder) {
    ...
    elementsFromWSDL = this.getTypesFromDefinitions(jsonSchema, elementTypesFromWSDL,
      definitionsNames, parserPlaceholder);
    complexTypes = this.getTypesFromDefinitions(jsonSchema, complexTypesFromWSDL,
      definitionsNames, parserPlaceholder);
    simpleTypes = this.getTypesFromDefinitions(jsonSchema, simpleTypesFromWSDL,
      definitionsNames, parserPlaceholder);

    if (complexTypes && complexTypes.length > 0) {
      this.processComplexOrSimpleTypesToElement(complexTypes, complexTypes,
        simpleTypes, targetNamespace, complexTypeElements);
    }
    if (simpleTypes && simpleTypes.length > 0) {
      this.processComplexOrSimpleTypesToElement(simpleTypes, complexTypes,
        simpleTypes, targetNamespace, simpleTypeElements);
    }
    if (elementsFromWSDL && elementsFromWSDL.length > 0) {
      this.processElementsToWSDElements(elementsFromWSDL, complexTypes,
