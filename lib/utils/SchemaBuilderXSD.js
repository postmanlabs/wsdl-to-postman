...
class SchemaBuilderXSD {
          ...
          keys.forEach((key) => {
            if (properties[key]) {
              ...
              if (this.isScalarType(type, hasRef)) {
                ...
                if (!this.isSelfReferenceElement(property.properties[key], rootElement.name ?
  ...
  handleOneOfAnyOf(oneOfOrAnyOf, currentSchemaProperty) {
    ...
    if (!complexProperty && !currentSchemaProperty[oneOfOrAnyOf][0].$ref) {
      this.handleChoiceAnyOf(currentSchemaProperty, oneOfOrAnyOf);
    }
  ...
  handleChoiceAnyOf(currentSchemaProperty, oneOfOrAnyOf) {
    let propName = Object.keys(currentSchemaProperty[oneOfOrAnyOf][0].anyOf[0].oneOf[0].properties)[0],
      reference = currentSchemaProperty[oneOfOrAnyOf][0].anyOf[0].oneOf[0].properties[propName].$ref;
    if (currentSchemaProperty.properties) {
      currentSchemaProperty.properties[propName] = { $ref: reference };
      currentSchemaProperty[oneOfOrAnyOf] = undefined;
    }
  }
  ...
  processReferenceTypeOfSchema(property, hasReferenceTypeKey, complexTypes, simpleTypes, elements) {
    ...
    if (complexType) {
      ...
      if (property.name !== property.properties.name &&
        !this.isSelfReferenceElement(property.properties, type, this.isSelfReferenceElementExactNameComparer)) {
   ...
   */
  isSelfReferenceElement(elementProperties, complexTypeName, comparer) {
