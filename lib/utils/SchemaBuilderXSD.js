...
const {
  getNodeName
} = require('xsd2jsonschema/src/xmlschema/xsdFileXmlDom');
...
class SchemaBuilderXSD {
  ...
  getElements(parsedXml, principalPrefix, wsdlRoot, schemaNamespace, tnsNamespace, parserPlaceholder) {
    ...
    let elementsFromMessages = this.getElementsFromMesages(parsedXml, principalPrefix,
      elements, {}, {}, parserPlaceholder, wsdlRoot);
    return elements.concat(elementsFromMessages);
  }

  getElementsFromMesages(parsedXML, principalPrefix, elementsInTypes, simpleTypes, complexTypes, parserPlaceholder, wsdlRoot) {
    let filtered = '',
      newElements = [];
    ...
    filtered.forEach((message) => {
      ...
      let parts = getXMLNodeByName(message, principalPrefix, ATRIBUTE_PART);

      parts.forEach((part) => {
        let realPartTypeorelement = ''
        let partType = getXMLAttributeByName(part, parserPlaceholder, ATRIBUTE_TYPE)
        let partName = getXMLAttributeByName(part, parserPlaceholder, ATRIBUTE_NAME)
        if (!partType) {
        ...
        if (realPartTypeorelement) {
          if (isKnownType(realPartTypeorelement)) {
            let element = new Element();
            element.name = partName;
            element.type = realPartTypeorelement.split(':')[1];
            ...
            type = this.getComplexTypeByName(realPartTypeorelement, complexTypes);
            if (type) {
              let element = new Element();
              ...
              type = this.getSimpleTypeByName(realPartTypeorelement, simpleTypes);
              if (type) {
                let element = new Element();
                element.name = type.name;
                element.type = type.type;
                ...
                type = elementsInTypes.find((element) => {
                  element.name === realPartTypeorelement
                });
                if (type) {
                  let element = new Element();
                  element.name = type.name;
  ...
  searchAndAssignComplex(rootElement, parsedSchema, complexTypes, simpleTypes) {
    traverseUtility(rootElement).forEach((x) => {
      if (x) {
        ...
        if (isArray) {
          if (x[isArray][0].$ref) {
            x.$ref = x[isArray][0].$ref;
            x[isArray] = undefined;
          }
          else {
            x.type = x[isArray][0].type;
            x.minLength = x[isArray][0].minLength;
            x.maxLength = x[isArray][0].maxLength;
            x.maximum = x[isArray][0].maximum;
            x.minimum = x[isArray][0].minimum;
            x.pattern = x[isArray][0].pattern;
            x.$ref = undefined;
          }
        }
        hasNonKnownType = Object.keys(x)
          .find(
            (key) => {
              return key === '$ref' && x[key] !== undefined;
            }
          );
        ...
        if (hasNonKnownType) {
          ...
          if (complexType) {
            ...
            this.searchAndAssignComplex(x.properties, parsedSchema, complexTypes, simpleTypes);
          }
          else {
