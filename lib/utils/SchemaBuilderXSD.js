...
class SchemaBuilderXSD {
  ...
  parseSchema(schema) {
    const xs2js = new Xsd2JsonSchema(),
      ...
      jsonChema = convertedSchemas['hello_world.xsd'].getJsonSchema();
    return jsonChema;
  }
  ...
  getXMLSchemaProcessed(schemaTag, schemaNamespace, tnsNamespace) {
    ...
    objectSchema = {};
    objectSchema[schemaNamespace.prefixFilter + 'schema'] = localSchemaTag;
  ...
  getElementsFromType(type, parsedSchema, schemaPrefix,
    ...
    const definitionsNames = Object.keys(parsedSchema.definitions),
      scTag = type[schemaPrefix + XML_NAMESPACE_SEPARATOR + SCHEMA_TAG];
    complexTypesFromWSDL = this.getArrayFrom(this.getComplexTypesFromSchema(scTag,
      schemaPrefix + XML_NAMESPACE_SEPARATOR));
