...
class BFSSoapParametersHelper {
  ...
  convertFromNodeToJson(root) {
    ...
    while (queue.length > 0) {
      ...
      if (!set.has(element)) {
        jobjRef = this.processFirstNode(element, jObj, jobjRef, set);
      }
      this.processChildren(element, jobjRef, set, queue, refQueue);
    }
  ...
  processChildren(element, jobjRef, set, queue, refQueue) {
  ...
  processFirstNode(element, jObj, jobjRef, set) {
