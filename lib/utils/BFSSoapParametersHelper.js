...
class BFSSoapParametersHelper {
    ...
    while (queue.length > 0) {
      ...
      for (let childrenIndex = 0; childrenIndex < element.children.length; childrenIndex++) {
        ...
        if (elementChild.isComplex) {
          ...
          let portion = this.getPreviousComplex(jObj, traverseOrder)
  ...
  getPreviousComplex(jObj, traverseOrder) {
    let previousComplex, jObjPortion;
    if (traverseOrder.length < 2) {
      return jObj
    }
    ...
    traverseUtility(jObj).forEach(function Found(property) {
      if (this.keys) {
        let found = this.keys.find((key) => {
          return key === previousComplex.name
        })
        if (found) {
          jObjPortion = property
        }
      }
    });
    return jObjPortion;
