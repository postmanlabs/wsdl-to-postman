...
class BFSSoapParametersHelper {
    ...
    while (queue.length > 0) {
      ...
      if (!set.has(element)) {
        if (element.isComplex) {
          ...
          jObj[envelopeAndProtocol][bodyAndProtocol][element.name] = this.getValueExample(element.type);
        }
        set.add(element);
      }
      for (let childrenIndex = 0; childrenIndex < element.children.length; childrenIndex++) {
        ...
        if (elementChild.isComplex) {
          ...
          this.assignPropertyValue(jObj, element.name, elementChild.name, this.getValueExample(elementChild.type));
        }
  ...
  /**
   * Gets a simple default value depending on the type
   * @param {string} type  the type of element in the message
   * @returns {*} value the example value default the one for string
   */
