    ...
    while (queue.length > 0) {
      ...
      if (!set.has(element)) {
        if (element.isComplex) {
          ...
          jObj[envelopeAndProtocol][bodyAndProtocol][element.name] = 'The element or type could not be found';
          set.add(element);
        }
        else {
          jObj[envelopeAndProtocol][bodyAndProtocol][element.name] = getValueExample(element);
        }
        set.add(element);
      }
      for (let childrenIndex = 0; childrenIndex < element.children.length; childrenIndex++) {
        ...
        else if (elementChild.type === 'error') {
