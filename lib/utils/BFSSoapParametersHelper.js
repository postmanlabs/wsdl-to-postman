...
class BFSSoapParametersHelper {
  ...
  convertFromNodeToJson(root) {
    ...
    while (queue.length > 0) {
      ...
      element['father'] = traverseOrder[traverseOrder.length - 1] ? traverseOrder[traverseOrder.length - 1].name : ''
      ...
      for (let childrenIndex = 0; childrenIndex < element.children.length; childrenIndex++) {
        ...
        if (elementChild.isComplex) {
          ...
          let portion = this.getPreviousComplex(jObj, element.father)
          this.assignPropertyValue(portion, element.name, elementChild.name, getValueExample(elementChild));
        }
  ...
  getPreviousComplex(jObj, fatherName) {
    let previousComplex, jObjPortion;
    if (!fatherName) {
      return jObj
    }
    // for (let i = traverseOrder.length - 2; i >= 0; i--) {
    //   if (traverseOrder[i].isComplex) {
    //     previousComplex = traverseOrder[i];
    //     break;
    //   }
    // }

    traverseUtility(jObj).forEach(function Found(property) {
      if (this.keys) {
        let found = this.keys.find((key) => {
          return key === fatherName
        })
        if (found) {
          jObjPortion = property[fatherName]
        }
      }
    });
    return jObjPortion;
