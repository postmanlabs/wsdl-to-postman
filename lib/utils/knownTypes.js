 ...
 * @returns {boolean} if is a date pattern
 */
function isTimePattern(element) {
  return element.pattern === TIME_PATTERN;
}
...
function getFromPattern(element) {
  ...
  if (isTimePattern(element)) {
    return getFromTime(element);
  }
  try {
    return new RandExp(
      element.pattern
...
/**
 * Returns an example of the element information when is string and uses
 * encoding 7bit, 8bit, binary, quoted-printable, base16, base32, and base64
 * @param {Element} element the element for getting and example
 * @returns {string} the generated example
 */
function getFromEncoding(element) {
  const content = 'content';
  if (element.contentEncoding === 'base64') {
    return Buffer.from(content).toString('base64');
  }
  if (element.contentEncoding === 'base16') {
    return Buffer.from(content).toString('hex');
  }
  if (element.contentEncoding === 'binary') {
    return Buffer.from(content).toString('binary');
  }
  return `could not encode with that encoding value: ${element.contentEncoding}`;
}
...
function getFromString(element) {
  ...
  if (element.contentEncoding) {
    return getFromEncoding(element);
  }
  if (element.enum) {
    return element.enum[0];
  }
