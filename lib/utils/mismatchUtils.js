const { XMLXSDValidator } = require('../xsdValidation/XMLXSDValidator'),
  ...
  INVALID_BODY_PATTERN_NOT_ALLOWED =
    /Element \'(.*)\': Character content is not allowed, because the content type is empty/,
  INVALID_BODY_PATTERN_INVALID_XML_STRING = /Error validating message/,
  INVALID_BODY_PATTERN_INVALID_ROOT =
    /Element \'(.*)\': No matching global declaration available for the validation root/,
  GLOBAL_BODY_ERROR_PATTERNS = [
    INVALID_BODY_PATTERN_INVALID_XML_STRING,
    INVALID_BODY_PATTERN_INVALID_ROOT
  ];
 ...
 * @param {boolean} hasChildren if the keyXpath related element has children or not
 * @returns {string} the specific element from the key in the message or empty string
 */
function getElementChildrenAsString(element, hasChildren) {
  let result;
  if (hasChildren) {
...
function getMismatchReasonCode(error, detailedBlobValidation, isResponse) {
  const NOT_EXPECTED_MESSAGE_PART = 'This element is not expected',
    MISSING_REQUEST_MESSAGE_PART = 'Missing child element(s)',
    bodyErrorCodes = [1841, 1, 1845],
    isTypeInvalidError = error.code === 1824,
    isNotExpectedInBodyError = error.code === 1871 && error.message.includes(NOT_EXPECTED_MESSAGE_PART),
    isMissingRequestError = error.code === 1871 && error.message.includes(MISSING_REQUEST_MESSAGE_PART),
    isInvalidBodyError = bodyErrorCodes.includes(error.code);
...
/**
 * get the element that matches with the provided xpath and find if it is in
 * request body but not in expected body, so returns the result
 ...
 */
function getElementFromNotExpectedInBodyByXpath(currentBody, cleanBody, xpath) {
 ...
 * (if is missing in request or invalid body(not expected in body) error it drives to the paren element)
...
function getSuggestedFixFromCurrentValue(node, currentValue, reasonCode) {
  ...
  else if ([INVALID_BODY_REASON_CODE, INVALID_RESPONSE_BODY_REASON_CODE].includes(reasonCode)) {
    return removeNodeFromString(node, currentValue);
  }
}

/**
 * reads the invalid body reason  when the type of error is not_expected_in_body message and gets the necessary
 * data to find the mismatched element and returns the xpath
 ...
 */
function handleNotExpectedInBodyAndGetElementData(reason, currentBody, cleanBody) {
  const result = reason.match(NOT_EXPECTED_IN_BODY_PATTERN),
    name = result[1],
    elementXpath = `//${name}`,
    node = getElementFromNotExpectedInBodyByXpath(currentBody, cleanBody, elementXpath),
 ...
 * @param {string} currentBody The current body provided in request
 * @param {string} cleanBody Body generated from schema without namespaces neither envelopes
 * @returns {string} xpath corresponding to the element in reason
 */
function handleInvalidBodyAndGetElementData(reason, currentBody, cleanBody) {
  let elementData;
  if (reason.match(NOT_EXPECTED_IN_BODY_PATTERN)) {
    elementData = handleNotExpectedInBodyAndGetElementData(reason, currentBody, cleanBody);
  }
  else {
    const result = reason.match(INVALID_BODY_PATTERN_NOT_ALLOWED),
      name = result[1],
...
function getElementData(mismatch, currentBody, cleanBody) {
  ...
  if ([INVALID_BODY_REASON_CODE, INVALID_RESPONSE_BODY_REASON_CODE].includes(reasonCode)) {
    ...
    data = handleInvalidBodyAndGetElementData(reason, currentBody, cleanBody);
  }
...
function getMismatchWithSuggestedFix(mismatch, currentBody, bodyFromSchema, detailedBlobValidation) {
  ...
  const {
    ...
    isGlobalBodyError = GLOBAL_BODY_ERROR_PATTERNS.filter((pattern) => {
      return reason.match(pattern) !== null;
    }).length > 0;

  if (!detailedBlobValidation || isGlobalBodyError) {
    key = '//soap:Body';
    ...
    actualValue = getElementChildrenAsString(actualElement, elementData.keyIsItsParent);
    ...
    suggestedValue = elementData.keyIsItsParent ?
      ...
      getElementChildrenAsString(suggestedElement, elementData.keyIsItsParent);
  }
