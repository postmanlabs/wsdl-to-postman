...
async function resolveRemoteRefsNoMerge(input, xmlParser, options = {}) {
  ...
  if (options.resolveRemoteRefs) {
    try {
      await processRemoteReferences(content, xmlParser, options, downloadedReferences, origin);
...
/**
   ...
   * @returns {array} - {content, path}
   */
function getRootFilesFromData(input) {
  let rootsContentArray = [];
  input.rootFiles.forEach((rootFile) => {
    let foundInData = input.data.find((dataFile) => { return dataFile.path === rootFile.path; });
    if (foundInData) {
      rootsContentArray.push({ path: foundInData.path, content: foundInData.content });
    }
  });
  return rootsContentArray;
}
...
async function resolveRemoteRefsMultiFile(input, xmlParser, options) {
  const rootsWithContent = getRootFilesFromData(input);
  let remoteRefRes =
  await getRemoteReferencesArray(rootsWithContent, xmlParser, options),
