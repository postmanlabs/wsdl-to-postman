...
* @returns {undefined} nothing
*/
function processScehma(schemaAndInformation, independent, attributePlaceHolder, dependent, schemasToOrderInformation) {
  let imports,
  ...
  if (imports && imports.length > 0) {
    imports.forEach((importElement) => {
      ...
      if (toProcess) {
        processScehma(toProcess, independent, attributePlaceHolder, dependent, schemasToOrderInformation);
      }
    });
...
function orderSchemasAccordingToDependencies(schemasToOrderInformation, attributePlaceHolder) {
  ...
  schemasToOrderInformation.forEach((schemaAndInformation) => {
    processScehma(schemaAndInformation, independent, attributePlaceHolder, dependent, schemasToOrderInformation);
  });
