...
function scopedGetMinOccursOfElement(element) {
...
function scopedGetMaxOccursOfElement(element) {
...
function scopedIsKnownType(key) {
  let knownTypes = ['unsignedLong', 'string', 'decimal', 'int'];
  return knownTypes.includes(key.split(XML_NAMESPACE_SEPARATOR)[1]);
}


function getChildren(rootElement, wsdlElement, schemaPrefix) {
  let children = [];
  console.log('call to get children')

  traverseUtility(rootElement).forEach(function(x) {
    let scalarType = Object.keys(x)
      .find(
        (key) => {
          return key === PARSER_ATRIBUTE_NAME_PLACE_HOLDER + ATRIBUTE_TYPE && scopedIsKnownType(x[key]);
        }
      );
    if (scalarType) {
      ...
      element.children = [];
      element.minOccurs = scopedGetMinOccursOfElement(x);
      element.maxOccurs = scopedGetMaxOccursOfElement(x);
      children.push(element);
      wsdlElement.children = children;
      console.log(wsdlElement)
    }
    else {
      ...
      if (hasNonKnownType && x[PARSER_ATRIBUTE_NAME_PLACE_HOLDER + ATRIBUTE_TYPE] !== '') {
        ...
        getChildren(x[schemaPrefix + COMPLEX_TYPE_TAG], element, schemaPrefix);
...
class SchemaBuilder {
  ...
  getElements2(parsedXml, principalPrefix, wsdlRoot, schemaPrefix) {
    let elements = [];
    ...
    types.forEach((type) => {
      ...
      elementsFromWSDL.forEach((element) => {
        ...
        getChildren(element, wsdlElement, schemaPrefix);
