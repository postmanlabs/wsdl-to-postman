...
class WsdlToPostmanCollectionMapper {
  ...
  getUrlDataFromOperations(operations) {
    ...
    operations.forEach((operation) => {
      const {
        ...
        protocolCapitalized = protocol.charAt(0).toUpperCase() +
          protocol.slice(1);

      urlsData.push({
        name,
        protocol: protocolCapitalized,
        url
      });
    });
    return [...urlsData];
  ...
  getVariablesFromUrlDataList(urlDataList) {
    let variables = urlDataList.filter((urlData) => {
      ...
      return {
        key: `${urlData.name}${urlData.protocol}BaseUrl`,
        value: `${protocol}${host}`,
        urlData
      };
    });
  ...
  replaceVariableInUrl(operation, variablesData) {
    ...
    let variableInUrl = variablesData.find((variable) => {
      const protocolMatch = variable.urlData.protocol.toLowerCase() === protocol,
        nameMatch = variable.urlData.name === name;
      return protocolMatch && nameMatch;
    });
    if (variableInUrl) {
   ...
   * @returns {array} An array with all items generated in postman item definition format
   */
  createItemsFromOperations(operations, urlVariablesData, securityPolicyArray) {
