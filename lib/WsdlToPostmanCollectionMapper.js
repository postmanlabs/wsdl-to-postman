...
class WsdlToPostmanCollectionMapper {
  ...
  getPostmanCollection(options) {
    let postmanCollectionDefinition = this.generateMappingObject(this.wsdlObject, options),
   ...
   */
  generateMappingObject(wsdlObject, options) {
    let urls = this.getUrlFromOperations(wsdlObject.operationsArray),
      ...
      items = this.createItemsFromOperations2(groupsInfo, urlVariables),
  ...
  groupOperations(operations, options) {
    ...
    if (options.folderStrategy === 'Port/Endpoint') {
      key = 'portName';
    }
    else if (options.folderStrategy === 'Service') {
      key = 'serviceName';
    }

    const allKeys = [...new Set(operations.map((operation) => operation[key]))];
    let groups = [];
    allKeys.forEach((valueKey) => {
      let operationsByKey = operations.filter((operation) => operation[key] === valueKey);
      groups.push({
        groupName: valueKey,
  ...
  createItemsFromOperations2(groupsInfo, urlVariables) {
    let items = [];
