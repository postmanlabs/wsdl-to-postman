const { getProtocolAndHost } = require('./utils/urlUtils');
...
class WsdlToPostmanCollectionMapper {
  ...
  generateMappingObject(wsdlObject, options, name) {
    let urls = this.getUrlFromOperations(wsdlObject.operationsArray),
      ...
      items = this.createItemsFromOperationsFolderStrategy(groupsInfo, urlVariables, wsdlObject.securityPolicyArray),
  ...
  getVariablesFromUrlList(urlList) {
    let variables = urlList.filter((url) => {
      ...
      return {
        key: index > 0 ? `baseUrl${index}` : 'baseUrl',
        value: `${protocol}${host}`
      };
    });
    return variables;
   ...
   */
  createItemsFromOperationsFolderStrategy(groupsInfo, urlVariables, securityPolicyArray) {
    let items = [];
    if (!groupsInfo.isGrouped) {
      return this.createItemsFromOperations(groupsInfo.operations, urlVariables, securityPolicyArray);
    }
    groupsInfo.groups.forEach((group) => {
      let operationItems = this.createItemsFromOperations(group.operations, urlVariables, securityPolicyArray);
   ...
   * @returns {array} An array with all items generated in postman item definition format
   */
  createItemsFromOperations(operations, urlVariables, securityPolicyArray) {
    let items = operations.map((operation) => {
      let postmanItem = {
        ...
        response: this.getResponses(operation, securityPolicyArray)
      };
      return postmanItem;
    });
    return items;
   ...
   */
  getBodyMessage(nodeElement, securityPolicyArray, protocol) {
   ...
   */
  getResponses(operation, securityPolicyArray) {
    let incorrectResponse,
      correctResponse,
      responses = [];
    if (operation.output) {
      correctResponse = this.buildResponseObject(' response', 'OK', 200,
        operation, this.getBodyMessage(operation.input, securityPolicyArray, operation.protocol), operation.output);
      responses.push(correctResponse);
    }
    if (operation.fault) {
      incorrectResponse = this.buildResponseObject(' fault', 'not OK', 500,
        operation, 'incorrect body message', operation.fault, securityPolicyArray);
  ...
  buildResponseObject(sufixName, status, code, operation, requestBodyMessage,
    responseBodyElement, securityPolicyArray) {
    let response = new Response({
      ...
      body: this.getBodyMessage(responseBodyElement, securityPolicyArray, operation.protocol)
    });
