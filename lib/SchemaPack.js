const multifileUtils = require('./utils/multifile'),
  ...
  { getBundledFiles } = require('./bundleFiles'),
  { getRootFilesApiOutput } = require('./detectRoot'),
  { resolveRemoteRefsMultiFile } = require('./utils/WSDLRemoteResolver');
...
class SchemaPack {
  ...
  async detectRootFiles() {
    ...
    rootFiles = getRootFilesApiOutput(input.data, wsdlParser, this.xmlParser);
  ...
  async bundle() {
    const input = this.input,
      versionToUse = input.specificationVersion ?
    ...
    if (this.options.resolveRemoteRefs) {
      let downloadRefs = await resolveRemoteRefsMultiFile(input, this.xmlParser, this.options);
      input.data = input.data.concat(downloadRefs);
    }

    bundledOutput = getBundledFiles(input.data, wsdlParser, this.xmlParser, this.options,
      input.origin);
