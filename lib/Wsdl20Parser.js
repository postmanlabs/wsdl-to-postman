...
class Wsdl20Parser {
      ...
      bindingOperations.forEach((operation) => {
        let interfaceOperationDocumentation = {},
          ...
          service = this.getServiceByBindingName(getAttributeByName(binding, NAME_TAG),
            services, principalPrefix);
        interfaceOperationDocumentation = getNodeByName(interfaceOperation, principalPrefix, DOCUMENTATION_TAG) ?
          this.getDocumentationString(getNodeByName(portTypeOperation, principalPrefix, DOCUMENTATION_TAG)) : '';

        wsdlOperation.name = getAttributeByName(interfaceOperation, NAME_TAG);
        wsdlOperation.url = getAttributeByName(serviceEndpoint, ADDRESS_TAG);
  ...
  getBindingInfoFromBindinTag(binding, soapNamespace, soap12Namespace, httpNamespace) {
    ...
    if ((soapNamespace && type === soapNamespace.url) || (soap12Namespace && type === soap12Namespace.url)) {
      ...
      verb = getHttpVerb(getAttributeByName(binding, httpNamespace.key + ':' + ATRIBUTE_METHOD_DEFAULT));
    ...
    try {
      ...
      services.forEach((service) => {
        ...
        endpoint = endpoints.find((endpoint) => {
          return getAttributeByName(endpoint, BINDING_TAG) === 'tns:' + bindingName;
        });
        if (endpoint) {
