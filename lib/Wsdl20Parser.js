...
class Wsdl20Parser {
  ...
  getWsdlObject(xmlDocumentContent) {
    ...
    wsdlObject = this.assignOperations(wsdlObject, parsedXml, wsdlObject.schemaNamespace.key + XML_NAMESPACE_SEPARATOR,
      wsdlObject.schemaNamespace.url);
    return wsdlObject;
  }
   ...
   * @returns {object} the WSDLObject
   */
  assignOperations(wsdlObject, parsedXml, schemaPrefix, schemaURL) {
    let newWsdlObject = {
        ...wsdlObject
      },
      wsdlOperations = [];
    const principalPrefix = this.getPrincipalPrefix(parsedXml),
      ...
      elements = this.getElementsFromWSDL(parsedXml, principalPrefix, schemaPrefix, schemaURL);
   ...
   * @returns {[object]} the [elements] object
   */
  getElementsFromWSDL(parsedXml, principalPrefix, schemaPrefix, schemaURL) {
    return getElementsFromWSDL(parsedXml, principalPrefix, schemaPrefix, WSDL_ROOT, schemaURL);
  }
