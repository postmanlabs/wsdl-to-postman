 ...
 */
function processUsernameToken(tokenInfo, policyNs) {
 ...
 */
function processSAMLToken(tokenInfo, state, policyNs) {
  let includeToken = tokenInfo['@_sp:' + ATRIBUTE_INCLUDE_TOKEN],
    version = '',
    samlToken;
 ...
 */
function processTransportBinding(tokenInfo, transportBindingAssertion, policyNs) {
  let transportToken = '',
    algorithmSuite = '',
    layout = '',
  ...
  traverseUtility(tokenInfo).forEach(function(property) {
    if (property !== undefined) {
      if (this.key === 'sp:' + TRANSPORT_TOKEN_TAG) {
        ...
        if (transportToken === HTTPS_TOKEN_TAG) {
          transporTokenpolicy = HTTPS_TOKEN_POLICY_OPTIONS.find((policyOption) => {
            let policy = property[policyNs.prefixFilter + POLICY_TAG]['sp:' + HTTPS_TOKEN_TAG][policyNs.prefixFilter + POLICY_TAG];
            if (policy) {
      ...
      if (this.key === 'sp:' + ALGORITHM_SUITE_TAG) {
        algorithmSuite = property[policyNs.prefixFilter + POLICY_TAG]['sp:' + BASIC_256_TAG] === '' ? BASIC_256_TAG : '';
      }
    ...
    traverseUtility(securityParsedNode).forEach(function(property) {
      if (property !== undefined) {
        ...
        if (state === SUPPORTING_TOKEN_STATE && this.key === 'sp:' + USERNAME_TOKEN_TAG) {
          let tokenAssertion = processUsernameToken(property, securityNS);
        ...
        if (this.key === 'sp:' + TRANSPORT_BINDING_TAG) {
          transportBindingAssertion = processTransportBinding(property, transportBindingAssertion, securityNS);
