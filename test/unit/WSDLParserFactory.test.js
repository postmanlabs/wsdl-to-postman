...
describe('Parser Factory getParserSafe', function () {

  it('should get version an object of type Wsdl11Parser when the input contains definitions>', function () {
    const factory = new WSDLParserFactory(''),
      concreteParser = factory.getParserSafe(`<?xml version="1.0" encoding="utf-8"?>
        <?xml-stylesheet type="text/xsl" href="http://tomi.vanek.sk/xml/wsdl-viewer.xsl"?>
        <wsdl:definitions xmlns:soap="http://schemas.xmlsoap.org/wsdl/soap/">
            <wsdl:types>
                <s:schema elementFormDefault="qualified" targetNamespace="http://tempuri.org/">
  ...
  it('should return undefined when input is empty string', function () {
    const factory = new WSDLParserFactory(),
      res = factory.getParserSafe('');
    expect(res).to.be.undefined;
  });

  it('should return undefined when input is an empty string', function () {
    const factory = new WSDLParserFactory(),
      res = factory.getParserSafe();
    expect(res).to.be.undefined;
  });

  it('should return undefined when input is null', function () {
    const factory = new WSDLParserFactory(),
      res = factory.getParserSafe(null);
    expect(res).to.be.undefined;
  });

  it('should return undefined is undefined', function () {
    const factory = new WSDLParserFactory(),
      res = factory.getParserSafe(undefined);
    expect(res).to.be.undefined;
  });

  it('should throw an error when input does not have any of the tags', function () {
    const factory = new WSDLParserFactory();
    let res = factory.getParserSafe(`<user is='great'>
      <name>Tobias</name>
      <familyName>Nickel</familyName>
      <profession>Software Developer</profession>
      <location>Shanghai / China</location>
